...
// Configure for this specific view (Development Notes)
const PAGE_TITLE = '开发笔记'
const PAGE_COVER = props.coverImage || 'https://bu.dusays.com/2026/02/04/698346b2079d6.jpg'
  ...
  try {
    ...
    if (cateCountRes.data) {
      ...
      if (Array.isArray(cateCountRes.data)) {
        cateCountRes.data.forEach((item: { name: string; count: number }) => {
          if (item.name) counts[item.name] = item.count || 0
        })
      }
    ...
    if (tagCountRes.data) {
      ...
      if (Array.isArray(tagCountRes.data)) {
        tagCountRes.data.forEach((item: { name: string; count: number }) => {
          if (item.name) counts[item.name] = item.count || 0
        })
      }
    ...
    if (targetCateId) {
      ...
      console.warn(`Category "${PAGE_TITLE}" not found.`)
      articles.value = []
      total.value = 0
    }
  } catch (err) {
...
<template>
  <div
    class="note-view min-h-screen bg-gray-50 dark:bg-[#1a1b26] text-gray-900 dark:text-[#c0caf5] font-sans transition-colors duration-300"
  >
    <Starry />
